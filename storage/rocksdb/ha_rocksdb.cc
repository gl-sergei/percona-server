...
static int rocksdb_remove_checkpoint(const char *checkpoint_dir_raw) {
  ...
  LogPluginErrMsg(INFORMATION_LEVEL, 0,
                  ...
                  checkpoint_dir.c_str());
  auto status = rocksdb::DestroyDB(checkpoint_dir, rocksdb::Options());
  int rc MY_ATTRIBUTE((__unused__));
  if (status.ok()) {
    return HA_EXIT_SUCCESS;
  }
...
struct rocksdb_ha_data {
  char *checkpoint_dir;
  Rdb_transaction *trx;
  bool disable_file_deletions;
  rocksdb_ha_data() : checkpoint_dir(), trx(), disable_file_deletions() {}
...
static rocksdb_ha_data *&get_ha_data(THD *const thd) {
  rocksdb_ha_data **ha_data = reinterpret_cast<rocksdb_ha_data **>(
      my_core::thd_ha_data(thd, rocksdb_hton));
  if (*ha_data == nullptr) {
    *ha_data = new rocksdb_ha_data();
  }
  return *ha_data;
}

static void destroy_ha_data(THD *const thd) {
  rocksdb_ha_data *&ha_data = get_ha_data(thd);
  delete ha_data;
  ha_data = nullptr;
}

static Rdb_transaction *&get_tx_from_thd(THD *const thd) {
  return get_ha_data(thd)->trx;
}
...
static int rocksdb_close_connection(handlerton *const hton, THD *const thd) {
  ...
  if (get_ha_data(thd)->disable_file_deletions) {
...
static int rocksdb_create_temporary_checkpoint_validate(
  ...
  if (current_checkpoint_dir != nullptr && new_checkpoint_dir != nullptr) {
    std::string errmsg = "Checkpoint already exists in directory '" +
                         std::string(current_checkpoint_dir) + "'";
    *reinterpret_cast<const char **>(save) = current_checkpoint_dir;
    return HA_EXIT_FAILURE;
  } else if (new_checkpoint_dir != nullptr) {
    auto res = rocksdb_create_checkpoint(new_checkpoint_dir);
    if (res == HA_EXIT_SUCCESS) {
    ...
    auto res = rocksdb_remove_checkpoint(current_checkpoint_dir);
...
static void rocksdb_disable_file_deletions_update(
  ...
  if (val && !get_ha_data(thd)->disable_file_deletions) {
    rdb->DisableFileDeletions();
    get_ha_data(thd)->disable_file_deletions = true;
  } else if (!val && get_ha_data(thd)->disable_file_deletions) {
    rdb->EnableFileDeletions(false);
    get_ha_data(thd)->disable_file_deletions = false;
  }
}
